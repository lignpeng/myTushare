...
function readTextFile(code, callback) {
  let fpatch = 'index/kshape/data/'
  let type = '.json'
  let requestURL = fpatch + code + type
  alert(requestURL)
  ...
  rawFile.onreadystatechange = function() {
      if (rawFile.readyState === 4 && rawFile.status == "200") {
          callback('success');
          let str = JSON.parse(rawFile.responseText)
          alert(str)
      }
  }
  rawFile.send();
...
window.dd = function (code){
  ...
  let fpatch = 'index/kshape/data/'
  let type = '.json'
  let requestURL = fpatch + code + type  //'json/superheroes.json';
  alert(requestURL)
  //创建一个HTTP请求对象
